...
BingX иногда оставляет в каталоге рынок, у которого endpoint свечей возвращает код `109415` (`is pause currently`). Такой рынок не считается доступным токеном: бот один раз пишет короткое предупреждение, помещает его во временный runtime blacklist и не запрашивает повторно до перезапуска, при этом продолжая сканировать все остальные рынки.
